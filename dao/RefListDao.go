...
import (
	"api-reflist-migration-service/db"
...
type RefListDao struct {
}

func (d RefListDao) Paginate(pagenumber int64, nperpage int64) ([]model.ReferenceListMongo, error) {
	...
	var jobs []model.ReferenceListMongo
	for cur.Next(context.TODO()) {
		var job model.ReferenceListMongo
		err := cur.Decode(&job)
		if err != nil {
			return nil, err
		}
...
func (d RefListDao) GetCount() (int64, error) {
	db := db.GetMongoDB()
	return db.Collection(os.Getenv("DATA_MONGODB_COLLECTION")).CountDocuments(context.TODO(),bson.D{})
}
func (d RefListDao) BulkInsert(Entity []model.ReferenceList, nperpage int64) error {
	sqldb := db.GetMysqlDB()
